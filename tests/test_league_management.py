...
class TestLeagueService:
    ...
    def test_update_league_non_commissioner(self, league_service, test_user, test_user2):
        ...
        with pytest.raises(Exception) as exc_info:
            league_service.update_league(
                league_id=league.id,
                user=test_user2,  # Not commissioner
                name="Updated League",
            )
    ...
    def test_generate_new_invite_code(self, league_service, test_user):
        ...
        new_code = league_service.generate_new_invite_code(
            league_id=league.id,
            user=test_user,
        )
    ...
    def test_get_user_leagues(self, league_service, test_user, test_user2):
        ...
        league1 = league_service.create_league(
            name="Owned League",
            commissioner=test_user,
        )

        # Create another league and join as member
        league2 = league_service.create_league(
            name="Member League",
            commissioner=test_user2,
        )
