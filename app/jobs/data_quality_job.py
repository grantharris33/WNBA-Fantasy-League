...
async def run_daily_quality_checks() -> Dict[str, Any]:
    ...
    try:
        ...
        # Log summary
        _log_info(f"Daily quality checks completed: {results['passed']} passed, {results['failed']} failed, {results['errors']} errors")
    ...
    try:
        ...
        for rule_config in default_rules:
            try:
                ...
                _log_error(f"Failed to create validation rule for {rule_config['entity_type']}.{rule_config['field_name']}: {str(e)}")
...
async def cleanup_old_anomalies(days_to_keep: int = 30) -> int:
    ...
    try:
        ...
        deleted_count = (
            ...
            .filter(
                DataAnomalyLog.is_resolved == True,
                DataAnomalyLog.resolved_at < cutoff_date
            )
            .delete()
        )
