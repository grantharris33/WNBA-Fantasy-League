...
def _upsert_player(session, athlete: dict[str, Any]) -> models.Player:
    ...
    if player is None:
        player = models.Player(id=player_id, full_name=athlete["displayName"], position=athlete.get("position", {}).get("abbreviation"))
        session.add(player)
    else:
            ...
            for stat_block in team_block.get("statistics", []):
                for athlete_block in stat_block.get("athletes", []):
                    ...
                    if existing:
                        ...
                        session.add(
                            models.StatLine(player_id=player.id, game_date=game_date, **stat_vals)
                        )
        session.commit()
